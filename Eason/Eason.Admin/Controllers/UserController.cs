...
namespace Eason.Admin.Controllers
{
    ...
    public class UserController : EBaseController
    {
        ...
        public JsonpResult List(int offset, int limit, string search)
        {
            ...
            {
                users = repository.GetAllList(offset, limit, m => m.isAdmin == 0);
            }
            else
            {
                users = repository.GetAllList(offset, limit, m => (m.name.Contains(search) || m.email.Contains(search) || m.telephone.Contains(search)) && m.isAdmin == 0);
            }
        ...
        [HttpPost]
        public async Task<ActionResult> Edit(long id, string email, string password, string telephone)
        {
            var user = await repository.GetAsync(id);
            if (user != null)
            {
